...
const initialState = {
  ...
  disableUndo: true,
  disableRedo: true,
  toggleMinusAdd: true, // false is Minus true is Add
  disableEdit: true,
  error: null,
  editableData: null,
};
...
export const editingSlice = createSlice({
  ...
  reducers: {
    ...
    toggleSelectElement: (state, action) => {
      ...
      const elementIndex = state.selectedElements.findIndex(
        (el) => el.id === elementId
      );
      if (elementIndex >= 0) {
        state.selectedElements.splice(elementIndex, 1);
        // Disable editing if the user clicks away
        if (state.editableData && state.editableData.id === elementId) {
    ...
    toggleShowForm: (state, action) => {
      ...
      state.toggleAddMinus = !state.toggleAddMinus;
    },
  },
});
